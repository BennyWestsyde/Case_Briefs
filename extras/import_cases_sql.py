...
if __name__ == "__main__":
    ...
    for case in cases.case_briefs:
        ...
        for subject in case.subjects:
            cursor.execute(
                "INSERT OR IGNORE INTO Subjects (name) VALUES (?)", (subject.name,)
            )
            connect.commit()
        ...
        for opinion in case.opinions:
            cursor.execute(
                "INSERT OR IGNORE INTO Opinions (author, opinion_text) VALUES (?, ?)",
                (opinion.author, opinion.text),
            )
            connect.commit()
            cursor.execute(
                "SELECT id FROM Opinions WHERE author = ? AND opinion_text = ?",
                (opinion.author, opinion.text),
            )
        ...
        for subject_id in subject_ids:
            cursor.execute(
                "INSERT OR IGNORE INTO CaseSubjects (case_label, subject_id) VALUES (?, ?)",
                (case.label.text, subject_id),
            )
        connect.commit()
        ...
        for opinion_id in opinion_ids:
            cursor.execute(
                "INSERT OR IGNORE INTO CaseOpinions (case_label, opinion_id) VALUES (?, ?)",
                (case.label.text, opinion_id),
            )
        connect.commit()
